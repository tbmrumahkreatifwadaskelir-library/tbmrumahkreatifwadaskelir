...
import { Search, Bell, Loader2, BookOpen, Menu, X, User } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm">
      <div className="max-w-[1440px] mx-auto flex items-center justify-between px-6 lg:px-10 h-16">
        ...
        <div className="flex items-center gap-3 ml-4">
          {isLoading ? (
            <div className="w-8 h-8 rounded-full border-2 border-[#99BD4A] border-t-transparent animate-spin" />
          ) : isLoggedIn ? (
            <>
              {/* Notification Bell */}
              <Link
            ...
            <>
              <Link
                href="/login"
                className="hidden lg:block text-sm font-bold text-slate-600 hover:text-[#99BD4A] transition-colors px-2"
              >
                Masuk
              </Link>
              <Link
                href="/login?view=register"
                className="flex items-center justify-center px-5 py-2.5 bg-[#99BD4A] hover:bg-[#87A840] text-white rounded-xl text-sm font-bold transition-all shadow-sm"
              >
                Daftar
              </Link>
            </>
